...
% Input parameters
Voo = fltcond.Voo;      % Voo - Undisturbed flow speed, m/s
rho = fltcond.rho;      % rho - air density, kg/m3
qoo = fltcond.qoo;      % qoo - dynamic pressure
...
% Calculating AIC matrix given the aerodynamics matrices
AIC = ( ((Voo/b)^2)*A1*(1i*k(i))^2 + (Voo/b)*A2*1i*k(i) + A3 )/(qoo);
